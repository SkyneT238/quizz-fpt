...
public class ChangePass extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        HashDAO hashPass = new HashDAO(oldpass);
        oldpass = hashPass.getHashCode().trim();
        if (userPass.equals(oldpass)) {
            Account updatedPass = dao.changePassword(userID, newpass);
            request.getSession().setAttribute("user", updatedPass);
            request.getRequestDispatcher("dashboardView.jsp").forward(request, response);
        } else {
            request.setAttribute("status", "error");
            request.setAttribute("title", "Oops...");
            request.setAttribute("message", "Your last password is not correct!");
